use std::{ops::Index, collections::HashMap, cmp::Ordering};
...
impl Index<Point> for Grid2d {
    ...
    fn index(&self, index: Point) -> &Self::Output {
        ...
        &self.grid[index.1 * self.size.0 + index.0]
    }
}

fn prepare_aoc_15a_input(filename: &str) -> Result<Grid2d, std::io::Error> {
...
fn solve_aoc_15a(grid: &Grid2d) -> usize {
    //println!("{:?}", grid);
...
fn main() -> Result<(), std::io::Error> {
    let input = prepare_aoc_15a_input("input/input.txt")?;
    let result = solve_aoc_15a(&input);
...
mod tests {
    ...
    #[test]
    fn test_aoc_15a() {
        let input = prepare_aoc_15a_input("input/test.txt").unwrap();
        let result = solve_aoc_15a(&input);
