...
fn prepare_aoc_04a_input(filename: &str) -> Result<(RandomSequence, Vec<Board>), std::io::Error> {
...
fn solve_aoc_04a(random_sequence: &RandomSequence, boards: &Vec<Board>) -> usize {
                ...
                Some(pos) => {
                    ...
                    if cols_status[num_of_board][col] == 0 || rows_status[num_of_board][row] == 0 {
                        return *number as usize *
                            board.iter().filter(|value| !value.1).fold(0, |acc, value| acc + value.0 as usize);
                    }
                },
                None => continue
            }
...
fn main() -> Result<(), std::io::Error> {
    let (random_sequence, boards) = prepare_aoc_04a_input("input/input.txt")?;
    let result = solve_aoc_04a(&random_sequence, &boards);
...
mod tests {
    ...
    #[test]
    fn test_aoc_04a() {
        let (random_sequence, boards) = prepare_aoc_04a_input("input/test.txt").unwrap();
        let result = solve_aoc_04a(&random_sequence, &boards);
